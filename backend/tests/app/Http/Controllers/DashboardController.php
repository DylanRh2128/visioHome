...
class DashboardController extends Controller
{
    ...
    public function getStats(Request $request)
    {
        try {
            ...
            // KPIS - Mezcla de acumulados y filtrados para mejor UX
            $income = Pago::where('estado', 'aprobado')->sum('monto') ?: 0;
            $salesCount = Pago::where('estado', 'aprobado')->count() ?: 0;
            $newUsers = Usuario::whereBetween('creado_en', [$start, $end])->count() ?: 0;
            $totalUsers = Usuario::count() ?: 0;
            $totalProperties = Propiedad::count() ?: 0;
            ...
            // Rankings con joins corregidos
            $topAgente = Agente::select(
                    'agentes.nombre', 
                    DB::raw('count(pagos.idPago) as total_sales'), 
                    DB::raw('sum(pagos.monto) as total_amount')
                )
                ->join('citas', 'agentes.docAgente', '=', 'citas.docAgente')
                ->join('pagos', 'citas.idCita', '=', 'pagos.idCita')
                ->where('pagos.estado', 'aprobado')
                ->groupBy('agentes.nombre', 'agentes.docAgente')
                ->orderBy('total_sales', 'desc')
                ->first();

            $topPropiedad = Propiedad::select('titulo', DB::raw('count(pagos.idPago) as total_sales'))
                ->join('pagos', 'propiedades.idPropiedad', '=', 'pagos.idPropiedad')
                ->where('pagos.estado', 'aprobado')
                ->groupBy('propiedades.titulo', 'propiedades.idPropiedad')
                ->orderBy('total_sales', 'desc')
                ->first();
            ...
            return response()->json([
                'kpis' => [
                    'income' => [
                        'current' => (float)$income, // Acumulado total histórico
                        'change' => 0
                    ],
                    'sales' => [
                        'current' => (int)$salesCount, // Acumulado total histórico
                        'change' => 0
                    ],
                    'users' => [
                        'current' => (int)$totalUsers, // Mostramos total para "Usuarios" en general
                        'new' => (int)$newUsers,
                        'change' => 0
                    ],
                    'properties' => [
                        'current' => (int)$totalProperties,
                        'change' => 0
                    ],
